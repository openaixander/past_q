...
def manage_uploads(request):
    """View for managing all uploaded content by the lecturer"""
    user = request.user
    lecturer_profile = get_object_or_404(LecturerProfile, user=user)
    ...
    return render(request, 'faculty/manage_uploads.html', context)
...
def delete_past_question(request, pk):
    ...
    try:
        ...
        messages.error(request, f'Error deleting past question: {str(e)}')
